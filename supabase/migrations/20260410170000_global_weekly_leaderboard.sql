-- Global weekly leaderboard for scored matches
-- Rankings are shared across all authenticated users and reset every Monday 00:00 UTC.

CREATE INDEX IF NOT EXISTS match_weekly_global_leaderboard_idx
  ON public.match (event_date DESC, user_id)
...
IS 'Returns the global weekly match leaderboard for the current Monday 00:00 UTC to next Monday 00:00 UTC window, including a wins ranking and a matches-played ranking.';
